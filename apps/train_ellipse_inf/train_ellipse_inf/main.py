"""
Local Ellipse Regression Training Script (Inference-Aligned)

This script trains an ellipse regression model for pupil detection on RAW images
without preprocessing (no gamma correction or CLAHE). The model learns to handle
raw camera input directly, making it suitable for deployment.
...
Usage:
    ...
    train-ellipse-inf --epochs 20 --batch-size 16 --lr 0.0005

    # Specify output directory
    train-ellipse-inf --output-dir ./checkpoints

    # Force CPU training
    train-ellipse-inf --device cpu

    # Disable MLflow logging
    train-ellipse-inf --no-mlflow
...
class IrisDataset(Dataset):
    """
    Inference-aligned IrisDataset.

    This dataset uses RAW images without any preprocessing (no gamma correction
    or CLAHE). The model learns to handle raw camera input directly, which
    ensures it works correctly at inference time without requiring preprocessing.

    Stochastic augmentations (line augment, gaussian blur, horizontal flip)
    are still applied during training to improve generalization.
    """
    ...
    def __getitem__(self, idx):
        ...
        # Image is RAW - model learns to handle raw input (no gamma/CLAHE preprocessing)
        ...
        # Image is RAW - no preprocessing applied
        # Only stochastic augmentations for training
        pilimg = image
...
def create_prediction_visualization(
    ...
    for i in range(num_samples):
        ...
        axes[i, 0].set_title("Input Image (RAW)", fontweight="bold")
        axes[i, 0].axis("off")
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train ellipse regression model for pupil detection (inference-aligned, no preprocessing)",
...
def main():
    ...
    print("=" * 80)
    print("Ellipse Regression Training (Inference-Aligned - No Preprocessing)")
    print("=" * 80)
    print(f"Using device: {device}")
    print("NOTE: Training on RAW images - model learns to handle raw input directly")
    ...
    print("NOTE: Using RAW dataset (no preprocessing applied to images)")
    ...
    print(f"  Preprocessing: NONE (inference-aligned)")
    print(f"{'='*80}")
    ...
    dataset_stats = {
        ...
        "preprocessing": "none (inference-aligned)",
    }
    ...
    if use_mlflow:
        ...
        mlflow_context = mlflow.start_run(run_name="ellipse-regression-training-inf-aligned")
    else:
    ...
    with mlflow_context as run:
            ...
            mlflow.set_tags(
                {
                    ...
                    "version": "v1.0-inference-aligned",
                    "preprocessing": "none",
                }
            )
        ...
        for epoch in range(EPOCHS):
            ...
            if miou_valid > best_valid_iou:
                ...
                best_epoch = epoch + 1
                best_model_path = os.path.join(args.output_dir, "best_ellipse_model_inf.pt")
            ...
            if (epoch + 1) % 10 == 0 or epoch == EPOCHS - 1:
                checkpoint_path = os.path.join(
                    args.output_dir, f"ellipse_model_inf_epoch_{epoch+1}.pt"
                )
        ...
        print("\n" + "=" * 80)
        print("Training completed! (Inference-Aligned)")
        ...
        print(f"Output directory: {args.output_dir}")
        print("NOTE: Model trained on RAW images (no preprocessing)")
        if use_mlflow:
