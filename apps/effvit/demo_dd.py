#!/usr/bin/env python3
"""
VisionAssist Live Demo - Dual Eye Segmentation (No Preprocessing)

This demo performs real-time semantic segmentation on BOTH eyes using webcam input.
It uses the TinyEfficientViT model without any image preprocessing (no CLAHE, no gamma).
...
class VisionAssistDemoDD:
    ...
    def _visualize(
        ...
        cv2.putText(
            annotated,
            "Mode: Dual Eye (No Preprocessing)",
    ...
    def run(self):
        ...
        print("\n" + "=" * 80)
        print("VisionAssist Live Demo - DUAL EYE (No Preprocessing)")
        print("=" * 80)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="VisionAssist Live Demo - Dual Eye Segmentation (No Preprocessing)"
    )
    parser.add_argument(
        "--model",
        type=str,
