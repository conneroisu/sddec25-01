...
class VisionAssistDemo:
    ...
    def _init_model(self):
        ...
        if self.verbose:
            print(f"Using device: {self.execution_provider}")

        # Load the model
        loaded_obj = torch.load(self.model_path, map_location=self.device)

        # Check if it's a state_dict or a complete model
        if isinstance(loaded_obj, dict):
            # It's a state_dict - create model and load weights
            self.model = ShallowNet(in_channels=1, out_channels=2)
        ...
        if self.verbose:
            print(f"Model loaded successfully on {self.execution_provider}")
            print(f"Model expects input: (B, C, W, H) = (1, 1, {self.MODEL_WIDTH}, {self.MODEL_HEIGHT})")
            print(f"Model outputs: (B, C, W, H) = (1, 2, {self.MODEL_WIDTH}, {self.MODEL_HEIGHT})")
    ...
    def _visualize(self, frame, eye_crop, mask, bbox, inference_time, face_detected):
        ...
        provider_short = "GPU" if self.execution_provider in ["CUDA", "MPS"] else "CPU"
        cv2.putText(
            annotated,
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to PyTorch model file (.pt or .pth) (REQUIRED)",
    )
    parser.add_argument(
        "--camera",
        type=int,
