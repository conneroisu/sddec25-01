#!/usr/bin/env python3
"""
Local training script for TinyEfficientViTSeg with Native Sparse Attention (NSA).

This script trains the sparse attention variant of TinyEfficientViTSeg for pupil
segmentation using the precomputed OpenEDS dataset from HuggingFace.

The model uses Native Sparse Attention (NSA) which combines:
1. Token Compression - Coarse-grained global context
2. Token Selection - Fine-grained important blocks
3. Sliding Window - Local context

Requirements:
- PyTorch with CUDA support (optional but recommended)
- HuggingFace datasets
- sparse/model.py in the same directory
...
# Import model and loss from local sparse model.py
from model import (
    TinyEfficientViTSeg,
    CombinedLoss,
)
...
def train(args):
    """
    Main training loop for sparse attention model.
    ...
    # ========================================================================
    # Initialize Model with NSA
    # ========================================================================

    print(
        "\nInitializing TinyEfficientViTSeg with Native Sparse Attention (NSA)..."
    )
    ...
    print(
        "Using Native Sparse Attention (NSA) with:"
    )
    print(
        "  - Token Compression for coarse-grained global context"
    )
    print(
        "  - Token Selection for fine-grained important blocks"
    )
    print(
    ...
    print("\n" + "=" * 80)
    print(
        "Starting Training with Native Sparse Attention"
    )
    print("=" * 80)
    ...
    for epoch in range(
        ...
        if valid_iou > best_iou:
            ...
            torch.save(
                ...
                f"{args.checkpoint_dir}/best_model_sparse.pth",
            )

            print(
                f"  >> Saved best sparse model with IoU={best_iou:.4f}"
            )
        ...
        if (epoch + 1) % 5 == 0:
            torch.save(
                ...
                f"{args.checkpoint_dir}/checkpoint_sparse_epoch_{epoch+1}.pth",
            )
            print(
    ...
    print(
        f"\nPyTorch checkpoint saved to: {args.checkpoint_dir}/best_model_sparse.pth"
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Train TinyEfficientViTSeg with Native Sparse Attention (NSA)",
    ...
    print("=" * 80)
    print(
        "TinyEfficientViTSeg with NSA - Training Configuration"
    )
    print("=" * 80)
