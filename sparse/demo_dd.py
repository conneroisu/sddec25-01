#!/usr/bin/env python3
"""
VisionAssist Live Demo - Dual Eye Segmentation with NSA (No Preprocessing)

This demo performs real-time semantic segmentation on BOTH eyes using webcam input.
It uses the TinyEfficientViT model with Native Sparse Attention (NSA) without any
image preprocessing (no CLAHE, no gamma).
...
class VisionAssistDemoDD:
    """Dual-eye demo application for VisionAssist live webcam inference with NSA."""
    ...
    def __init__(self, model_path: str, camera_index: int = 0, verbose: bool = False):
        """
        Initialize the VisionAssist dual-eye demo with NSA.
    ...
    def _visualize(
        ...
        if not face_detected:
            ...
            status_text = f"Both Eyes (NSA)" if eyes_detected == 2 else f"{eyes_detected} Eye (NSA)"
        ...
        cv2.putText(
            annotated,
            "Mode: Dual Eye NSA (No Preprocessing)",
    ...
    def run(self):
        ...
        print("\n" + "=" * 80)
        print("VisionAssist Live Demo - DUAL EYE NSA (No Preprocessing)")
        print("=" * 80)
        ...
        print(f"Mode: Both eyes, no preprocessing (no CLAHE, no gamma)")
        print("\nControls:")
        print("  ESC - Exit")
        ...
        cv2.namedWindow("VisionAssist Dual Eye NSA Demo", cv2.WINDOW_NORMAL)
        ...
        try:
            while True:
                if not self.paused:
                    ...
                    # Display
                    cv2.imshow("VisionAssist Dual Eye NSA Demo", annotated)
...
def main():
    """Main entry point for the VisionAssist dual-eye NSA demo."""
    parser = argparse.ArgumentParser(
        description="VisionAssist Live Demo - Dual Eye Segmentation with NSA (No Preprocessing)"
    )
    parser.add_argument(
        "--model",
        type=str,
