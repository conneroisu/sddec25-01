...
\begin{itemize}
	\item \textbf{Mean Total Latency}: Approximately 500--600 ms with low variance
	\item \textbf{Mean DPU Time}: Approximately 450--500 ms with minimal variance
	\item \textbf{Mean Preprocess}: Approximately 30--35 ms
	\item \textbf{Mean Postprocess}: Approximately 25--30 ms
	\item \textbf{Throughput}: Approximately 2 FPS
	\item \textbf{Memory Usage}: Approximately 40--45 MB
\end{itemize}

Single model provides stable baseline with low variance. DPU execution
dominates (approximately 88--90\% of latency), with pre/postprocessing contributing approximately 10--12\%.
...
\begin{itemize}
	\item \textbf{Mean Total Latency}: Approximately 4500--5500 ms with moderate variance
	\item \textbf{Mean DPU Time}: Approximately 4400--5400 ms with moderate variance
	\item \textbf{Mean Preprocess}: Approximately 30--35 ms
	\item \textbf{Mean Postprocess}: Approximately 25--30 ms
	\item \textbf{Throughput}: Approximately 0.2 FPS
	\item \textbf{Memory Usage}: Approximately 130--140 MB
\end{itemize}
...
Split model incurs approximately 800--900\% higher latency (approximately 9--10x slower) from segment coordination,
inter-segment data transfer, and compiler-induced scaling corrections.
Single model significantly outperforms the split architecture.
...
\begin{itemize}
	\item \textbf{Parallelization Overhead}: Naive algorithm division
	      introduces approximately 4000--4500ms overhead from segment coordination,
	      data transfer, and Vitis AI compiler scaling corrections
	\item \textbf{Memory Efficiency}: Single model: approximately 40--45MB vs split model:
	      approximately 130--140MB due to intermediate tensor storage between segments
	\item \textbf{Performance Ratio}: Single model is approximately 9--10x faster than
	      split model (DPU time ratio: approximately 0.10x)
	\item \textbf{Throughput Comparison}: Single model: approximately 2 FPS vs split
	      model: approximately 0.2 FPS
\end{itemize}
...
\begin{itemize}
	...
	\item \textbf{Accuracy Validation}: Current implementation achieves
	      approximately 98--99\% IoU accuracy, within target range of 99.8\%~\cite{wang2021}
...
\begin{itemize}
	\item \textbf{Training Time Reduction}: Complete 50-epoch training
	      completes in approximately 45--55 minutes on T4 GPU, representing approximately 15x
	      speedup over CPU-based baseline (approximately 12 hours)
	\item \textbf{Model Performance}: Achieved approximately 98--99\% mIoU on
	      validation set
	\item \textbf{Per-Class Performance}: Background IoU approximately 99\%, Pupil
	      IoU approximately 98--99\%, demonstrating balanced performance across minority
...
\begin{itemize}
	\item \textbf{VR vs.\ Medical Domain}: OpenEDS targets VR headset
	      ...
	      characteristics due to underlying conditions
	\item \textbf{Controlled Illumination}: VR headset near-infrared
	      ...
	      lighting in wheelchair environments
