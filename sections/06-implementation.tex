...
\item \textbf{DPU Utilization}: Both implementations show high DPU utilization (89.6\% for single model, 93.0\% for split model)
\item \textbf{Processing Consistency}: Single model shows more consistent performance with lower variance, important for real-time applications
\item \textbf{Memory Efficiency}: Single model maintains relatively low memory footprint (45 MB)
\end{itemize}
