...
dominates (89.6\% of latency), with pre/postprocessing contributing approximately 10\%.
...
Split model incurs 48.2\% higher latency from thread coordination and data
transfer. Increased variance ($\pm$3.39ms vs $\pm$0.26ms) indicates
...
\begin{itemize}
	\item \textbf{Parallelization Overhead}: Naive algorithm division
	      introduces 255ms overhead from thread synchronization and data transfer
	\item \textbf{Memory Efficiency}: Single model: 45MB vs split model:
	      111.01MB due to intermediate tensor storage between segments
\end{itemize}
...
training period. With GPU acceleration enabling batch sizes of 32, the
current training configuration uses 15 epochs.
...
Alpha scheduling implements curriculum learning, adapting to the total
number of epochs $E$:

\begin{equation}
	\alpha(t) =
	\begin{cases}
		1 - \frac{t}{\min(E, 125)} & \text{if } t \leq \min(E, 125) \\
		1                          & \text{if } t > \min(E, 125)
	\end{cases}
\end{equation}

For the GPU-accelerated training with batch size 32 and 15 epochs
($E=15$), alpha transitions linearly from approximately 0.93 (epoch 1)
to 0.0 (epoch 15) over all training epochs. Early training emphasizes
surface loss for boundary topology; later training focuses on dice
loss for segmentation quality. This prevents topologically incorrect
solutions difficult to correct later.
