...
Our implementation realizes the resource scheduling approach defined in Section~\ref{design:dpu-scheduling}:

\begin{itemize}
\item \textbf{DPU Access Management}: Sequential scheduling system implementing round-robin algorithm for fair DPU access allocation across semantic segmentation, blink detection, and eye tracking algorithms
\item \textbf{Thread Coordination}: Multi-threading framework (see Section~\ref{design:threading-model}) implemented for CPU-based preprocessing and postprocessing~\cite{park2022thread}
\item \textbf{Memory Management}: Optimized memory allocation strategy for efficient pipelined data flow with thread-local storage~\cite{chen2022memory}
\item \textbf{Synchronization}: Inter-thread communication mechanisms using mutexes and semaphores established for pipeline stage coordination
\end{itemize}
...
\item \textbf{Thread Synchronization}: Implementing the threading model (Section~\ref{design:threading-model}) while ensuring proper coordination between pipeline processing threads without deadlocks or race conditions
\item \textbf{Sequential DPU Scheduling}: Realizing the resource scheduling approach (Section~\ref{design:dpu-scheduling}) to efficiently balance semantic segmentation requirements with periodic data collection needs of auxiliary algorithms
\item \textbf{Performance Optimization}: Achieving target pipelined throughput for real-time operation while managing synchronization overhead
\end{itemize}
...
\item Optimize DPU scheduling system (Section~\ref{design:dpu-scheduling}) for maximum throughput
...
\item Complete integration testing with all auxiliary algorithms and DPU scheduling (Section~\ref{design:dpu-scheduling})
...
Our implementation follows the threading model defined in Section~\ref{design:threading-model}. Key implementation details include:

\begin{itemize}
\item \textbf{POSIX Threading}: Utilizes POSIX threads (pthreads) for multi-core ARM processing on the Kria KV260
\item \textbf{Performance Impact}: Thread synchronization overhead measured at 48.2\% latency increase in split model approach (see Section~\ref{subsec:performance-metrics}), demonstrating the importance of minimizing inter-thread coordination
\item \textbf{Pipeline Implementation}: Each pipeline stage runs in a dedicated thread with synchronization points between stages to ensure data consistency
\item \textbf{Resource Contention}: Higher variance in split model timings (±3.38ms vs ±0.05ms) indicates synchronization overhead and resource contention between parallel threads
\end{itemize}
