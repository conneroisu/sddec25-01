...
\begin{itemize}
	\item \textbf{AMD Kria KV260 Development Board}~\cite{xilinx2022kv260} (see Figure~\ref{fig:kv260})
	      \begin{itemize}
...
\begin{enumerate}
	\item \textbf{U-Net Semantic Segmentation Algorithm} (see Figure~\ref{fig:unet})
	      \begin{itemize}
...
The AMD Kria KV260 provides a balance of performance and power efficiency for our application:

\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.7\textwidth]{assets/kv260.png}
	\caption{AMD Kria KV260 Development Board featuring Zynq UltraScale+ MPSoC with integrated DPU for AI acceleration.}~\label{fig:kv260}
\end{figure}
...
\begin{figure}[htbp]
	...
	\caption{U-Net encoder-decoder architecture with skip connections for semantic segmentation. The architecture features contracting path (left) for context capture and expansive path (right) for precise localization.}~\label{fig:unet}
\end{figure}
...
\begin{figure}[H]
	...
	\caption{Xilinx Vitis AI Runtime (VART) stack architecture showing the layered approach from application level through runtime libraries to hardware acceleration.}~\label{fig:vart-stack}
\end{figure}

The VART stack architecture (Figure~\ref{fig:vart-stack}) illustrates the complete software stack from the application layer down to the Deep Learning Processing Unit (DPU) hardware. This layered approach enables efficient neural network inference by providing optimized runtime libraries and hardware abstraction layers that maximize utilization of the Kria KV260's acceleration capabilities.
