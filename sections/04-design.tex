...
The key innovation is parallelized CPU processing with efficient sequential
DPU scheduling on the AMD Kria KV260, which has limited memory and a single
DPU but powerful acceleration when properly leveraged. While the original
design concept targeted physical pipelining of model segments, hardware
constraints (single-threaded DPU, one xmodel at a time) necessitated a pivot
to parallelization with optimized sequential execution and overlapped
