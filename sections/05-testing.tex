...
\item Testing DPU access scheduling (Section~\ref{design:dpu-scheduling}) during development
\end{itemize}
...
\item Ensuring pipelined threads (Section~\ref{design:threading-model}) and DPU scheduling (Section~\ref{design:dpu-scheduling}) worked together correctly
...
\begin{itemize}
\item Resource allocation verification to confirm fair DPU access distribution per the scheduling approach (Section~\ref{design:dpu-scheduling})
...
\item \textbf{Thread Coordination:}
   \begin{itemize}
   \item Testing the threading model implementation (Section~\ref{design:threading-model}) for proper resource allocation management
   \item Verifying that synchronization primitives (mutexes, semaphores, condition variables) worked correctly for deadline-sensitive operations
   \item Validating coordination between algorithms with interdependencies without race conditions or deadlocks
   \end{itemize}
\end{enumerate}
...
\item Verified DPU scheduling (Section~\ref{design:dpu-scheduling}) worked correctly with FPGA programming
