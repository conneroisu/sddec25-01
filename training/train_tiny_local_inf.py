#!/usr/bin/env python3
"""
Local training script for TinyEfficientViT-Micro semantic segmentation - INFERENCE ALIGNED.

This is the inference-aligned version of the training script. It trains on RAW images
(no gamma correction or CLAHE preprocessing applied), so the model learns to handle
raw camera input directly. This ensures the trained model works correctly at inference
time without requiring preprocessing in the inference pipeline.
...
Usage:
    python train_tiny_local_inf.py --epochs 10 --batch-size 32
    python train_tiny_local_inf.py --device cuda --output-dir ./checkpoints
    python train_tiny_local_inf.py --resume                 # Auto-detect best model
    python train_tiny_local_inf.py --resume path/to/model.pt  # Specific checkpoint
    python train_tiny_local_inf.py --help
"""
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Train TinyEfficientViT-Micro for semantic segmentation (local version, inference-aligned)",
