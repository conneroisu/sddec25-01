"""
TinyEfficientViT Model Architecture

This module contains the complete TinyEfficientViT architecture for semantic segmentation.
The model is designed for efficient inference with <60k parameters, featuring:
- Lightweight patch embedding with stride-4 reduction
- Multi-stage encoder with cascaded group attention
- FPN-style decoder with skip connections

Classes:
    TinyConvNorm: Convolution + BatchNorm layer
    TinyPatchEmbedding: Lightweight patch embedding
    TinyCascadedGroupAttention: Efficient attention mechanism
    TinyLocalWindowAttention: Local window attention wrapper
    TinyMLP: Tiny MLP with expansion ratio
    TinyEfficientVitBlock: Single EfficientViT block
    TinyEfficientVitStage: Single stage with optional downsampling
    TinyEfficientVitEncoder: Complete encoder with 3 stages
    TinySegmentationDecoder: Lightweight FPN-style decoder
    TinyEfficientViTSeg: Complete model for semantic segmentation
"""
