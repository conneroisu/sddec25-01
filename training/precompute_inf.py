#!/usr/bin/env python3
"""
Dataset Precomputation Script for OpenEDS (Inference-Aligned)
...
NOTE: This version stores RAW images without gamma correction or CLAHE so that
the model learns to handle raw input directly. This aligns training with inference
where preprocessing may not be applied, improving model robustness.
...
3. Ellipse Parameter Extraction: Fit ellipse to pupil contour
4. Spatial Weights: Compute boundary weights using morphological gradient
5. Distance Map: Signed distance transform per class

Usage:
    # Full precompute and push to HuggingFace
    python precompute_inf.py --hf-repo Conner/sddec25-01

    # Local only (no HuggingFace push)
    python precompute_inf.py --no-push --output-dir ./my_dataset

    # Validate existing dataset
    python precompute_inf.py --validate --hf-repo Conner/sddec25-01

    # Skip Kaggle download (use existing files)
    python precompute_inf.py --skip-download --output-dir ./existing_data
"""
...
def preprocess_sample(
    ...
    5. Compute distance map

    NOTE: This version stores RAW images without gamma correction or CLAHE.
    ...
    # 3. Extract and normalize ellipse parameters
